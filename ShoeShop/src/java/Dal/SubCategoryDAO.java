...
//                s.setCategory(new CategoryDAO().getCategory(rs.getInt("CategoryId")));
    ...
    public void updateSubCate(String name, String des, String status, String subId) {
        try {
            String sql = "update SubCategories set SubCategoryName = ?, Description = ?, Status = ? where SubCategoryId = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public void insertSubCategory(String name, String des, String cateId) {
        try {
            String sql = "insert into SubCategories (SubCategoryName, Description, CategoryId, CreatedAt, Status) VALUES (?,?,?, GETDATE(),'activate')";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setString(1, name);
            stm.setString(2, des);
